...
export const ScraperTrigger: React.FC = () => {
  ...
  const updateKeywords = (region: 'eu' | 'tr', keywords: string) => {
    const setter = region === 'eu' ? setEuConfig : setTrConfig;
    const config = region === 'eu' ? euConfig : trConfig;
  ...
  const RegionPanel = ({ config, region }: { config: ScraperRegionConfig; region: 'eu' | 'tr' }) => {
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleTrigger(region);
              }}
              disabled={enabledCount === 0 || triggering === region}
          ...
          <div className="border-t border-slate-700 p-4 space-y-4 animate-fade-in">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateKeywords(region, e.target.value)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {config.sources.map((source) => (
                  <div
                    ...
                    onClick={() => toggleSource(region, source.id)}
                  >
                    <input
                      type="checkbox"
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const setter = region === 'eu' ? setEuConfig : setTrConfig;
                  setter({ ...config, scrapeDepth: parseInt(e.target.value) });
                }}
  ...
  return (
    <div className="space-y-4">
      ...
      <RegionPanel config={euConfig} region="eu" />
      <RegionPanel config={trConfig} region="tr" />
    </div>
  );
};
